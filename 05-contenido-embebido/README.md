...
- [ ] Estructura semántica adecuada que organice la página con `<header>`, `<main>`, `<section>` y `<footer>`.
- [ ] **Cabecera:** Encabezado `<h1>` con el título del documental y una imagen principal optimizada que use `src`, `alt` descriptivo, dimensiones fijas (`width`, `height`) y `loading="lazy"`.
- [ ] **Sección de galería / fotografía:** Al menos una imagen formal dentro de `<figure>` con su respectivo `<figcaption>` que indique la ubicación geográfica y crédito fotográfico.
- [ ] **Sección de afiche adaptable:** Un bloque `<picture>` que ofrezca:
...
- [ ] **Sección de trailer y banda sonora:**
...
- [ ] **Sección de locación:** Un mapa incrustado con `<iframe>` (OpenStreetMap o Google Maps) que incluya obligatoriamente el atributo descriptivo `title` y `loading="lazy"`.
