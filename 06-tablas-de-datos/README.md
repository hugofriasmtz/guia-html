...
- [x] Estructura base completa de HTML5 con `<!DOCTYPE html>`, `<html>`, `<head>` y `<body>`.
- [x] Un encabezado principal `<h1>Planes y Tarifas del Servicio Cloud</h1>`.
- [x] Una etiqueta `<table>` con un `<caption>` claro y descriptivo.
- [x] Un bloque `<colgroup>` que diferencie la columna de características de las columnas de los planes.
- [x] Una sección `<thead>` con los nombres de las columnas:
  - *Características*, *Plan Básico*, *Plan Pro*, *Plan Enterprise*.
  - Todos los títulos con su atributo `scope="col"`.
- [x] Una sección `<tbody>` con al menos 4 filas de características:
...
- [x] Una sección `<tfoot>` que resuma el costo mensual o la garantía de reembolso:
  - Debe usar `colspan` para alinear la leyenda con los valores finales.
- [x] Estructura semántica impecable: sin celdas desalineadas ni filas rotas por mal cálculo de fusiones.
