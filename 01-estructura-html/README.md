...
- [ ] El archivo debe llamarse exactamente `reto.html` y estar ubicado directamente dentro de `01-estructura-html/`.
- [ ] **Escrito a mano:** Escribe cada etiqueta carácter por carácter (no uses el atajo `!` de Emmet) para interiorizar la sintaxis.
- [ ] La primera línea debe ser la declaración de estándar: `<!DOCTYPE html>`.
- [ ] El elemento raíz `<html>` debe incluir el atributo de idioma en español: `lang="es"`.
- [ ] **Configuración en `<head>`:**
  - [ ] Codificación de caracteres universales con `<meta charset="UTF-8">`.
  - [ ] Configuración responsiva con `<meta name="viewport" content="width=device-width, initial-scale=1.0">`.
  - [ ] Título de pestaña personalizado con el formato: `<title>Mi Portafolio Profesional | Inicio</title>`.
- [ ] **Contenido en `<body>`:**
  - [ ] Un encabezado `<h1>` con tu nombre completo o nombre del proyecto.
  - [ ] Un párrafo `<p>` que contenga una breve descripción de bienvenida y al menos una palabra dentro de `<strong>`.
  - [ ] Al menos un comentario descriptivo `<!-- ... -->` que indique dónde colocarás tus proyectos en las siguientes lecciones.
- [ ] **Calidad de código:**
  - [ ] Cero cruces de etiquetas (anidamiento perfecto de cajas).
  - [ ] Indentación limpia y uniforme de 2 espacios por cada nivel jerárquico.
- [ ] **Comprobación:**
  - [ ] Abre `reto.html` con **Live Preview: Show Preview** (o en el navegador con `F12`) y confirma que se visualice tu árbol DOM sin errores ni etiquetas fuera de lugar.
